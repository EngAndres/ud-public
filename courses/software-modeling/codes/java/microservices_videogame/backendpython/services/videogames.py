...
class VideoGameServices:
    ...
    def get_by_name_category(self, name_category: NameCategoryDTO) -> List[VideoGamesDAO]:
        ...
        response = []
        for videogame in self.repository.get_videogames():
            if name_category.name.lower() in videogame.name.lower() and name_category.category.lower() in videogame.category.lower():
                response.append(videogame)
        return response